...
def test_bad_gstin_creates_issue(monkeypatch):
    monkeypatch.setattr(
        extract_router,
        "extract_invoice",
        lambda d, k: fake(supplier_gstin="27ABCDE1234F1Z5"),  # fails checksum
    )
    headers, invoice_id = upload_one()
    client.post(f"/invoices/{invoice_id}/extract", headers=headers)
    r = client.get(f"/invoices/{invoice_id}/issues", headers=headers)
...
def test_tax_mismatch_creates_error_severity_issue(monkeypatch):
    monkeypatch.setattr(
        extract_router,
        "extract_invoice",
        lambda d, k: fake(taxable_value=1000, cgst=90, sgst=90, igst=0, total_amount=5000),
    )
    headers, invoice_id = upload_one()
    client.post(f"/invoices/{invoice_id}/extract", headers=headers)
    r = client.get(f"/invoices/{invoice_id}/issues", headers=headers)
...
def test_reextract_replaces_old_issues(monkeypatch):
    ...
    monkeypatch.setattr(
        extract_router, "extract_invoice", lambda d, k: fake(supplier_gstin="27ABCDE1234F1Z5")
    )
    client.post(f"/invoices/{invoice_id}/extract", headers=headers)
    ...
    monkeypatch.setattr(
        extract_router, "extract_invoice", lambda d, k: fake(supplier_gstin="27AAPFU0939F1ZV")
    )
    client.post(f"/invoices/{invoice_id}/extract", headers=headers)
